...
public class InventoryPouchSmall implements ISidedInventory {
  ...
  protected void loadNBT() {
    ...
    final NBTTagList nbtTagList = tagCompound.getTagList(NBT_TAG_ITEMLIST, Constants.NBT.TAG_COMPOUND);
    for (int tagCount = nbtTagList.tagCount() - 1; tagCount >= 0; --tagCount) {
      final NBTTagCompound itemEntry = nbtTagList.getCompoundTagAt(tagCount);
