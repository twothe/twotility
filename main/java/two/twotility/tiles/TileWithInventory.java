...
public abstract class TileWithInventory extends TileEntity implements ISidedInventory {
  ...
  public ItemStack getStackInSlot(final int slot) {
    if (slot < 0) {
      FMLLog.log(TwoTility.MOD_ID, Level.WARN, "Requested illegal slot item #%d < 0", slot);
      return null;
    } else if (slot < inventory.length) {
      return inventory[slot];
    } else {
      FMLLog.log(TwoTility.MOD_ID, Level.WARN, "Requested illegal slot item #%d > total size", slot);
      return null;
    }
  }
  ...
  public boolean isUseableByPlayer(final EntityPlayer player) {
    ...
    if (player != null) {
      ...
      FMLLog.log(TwoTility.MOD_ID, Level.WARN, "Requested isUseableByPlayer with null player");
      return false;
    }
  }
  ...
  protected static void readInventoryFromNBT(final NBTTagCompound tagCompound, final String inventoryName, final ItemStack[] inventory) {
    final NBTTagList inventoryList = tagCompound.getTagList(inventoryName, Constants.NBT.TAG_COMPOUND); // we have a list of compounds
    for (int tagCount = inventoryList.tagCount() - 1; tagCount >= 0; --tagCount) {
      final NBTTagCompound itemEntry = inventoryList.getCompoundTagAt(tagCount);
  ...
  public boolean hasCustomInventoryName() {
    return false; // this cannot be named by default
  }
