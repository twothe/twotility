  ...
  public Object getServerGuiElement(final int ID, final EntityPlayer player, final World world, final int x, final int y, final int z) {
    if ((ID >= 0) && (ID < knownGuis.size())) {
      ...
      try {
        ...
        FMLLog.log(TwoTility.MOD_ID, Level.WARN, e.getMessage());
        world.removeTileEntity(x, y, z);
      } catch (Exception e) {
        FMLLog.log(Level.WARN, e, "Unable to create container for GUI: %d", ID);
      }
    } else {
      FMLLog.log(Level.WARN, "Received server request for unknown Gui ID: %d", ID);
    }
    return null;
  }
  ...
  public Object getClientGuiElement(final int ID, final EntityPlayer player, final World world, final int x, final int y, final int z) {
    if ((ID >= 0) && (ID < knownGuis.size())) {
      ...
      try {
        ...
        FMLLog.log(Level.WARN, e, "Unable to create GUI: %d", ID);
      }
    } else {
      FMLLog.log(Level.WARN, "Received client request for unknown Gui ID: %d", ID);
    }
    return null;
  }
}
