    ...
    public StreamRunnable(final URL url, final PaulsCodeDevice device) {
      ...
      if (this.device.getAudioFormat() == null) { // PaulsCode requires the AudioFormat to be known. Must stop the game here till it is received. Device uses internal caching to avoid this if possible.
        try {
          ...
          FMLLog.log(TwoTility.MOD_ID, Level.SEVERE, "Failed to initialize player (%s): %s", ex.getClass().getSimpleName(), ex.getMessage());
    ...
    public void run() {
      try {
        ...
        try {
          ...
          FMLLog.log(TwoTility.MOD_ID, Level.SEVERE, "Failed to play (%s): %s", e.getClass().getSimpleName(), e.getMessage());
        } finally {
          player.close();
        }
      } catch (Exception ex) {
